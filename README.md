...
**Report Server** is a background process module for the [Apostol](https://github.com/apostoldevel/apostol) framework. It runs as an independent forked process and executes pre-defined database reports asynchronously. A report is a PL/pgSQL routine registered in the report queue; the process picks it up, runs it as a PostgreSQL query, and updates the report's state when done.
